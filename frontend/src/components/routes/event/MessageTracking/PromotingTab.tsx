...
import {useGetPromotingMessages, GET_PROMOTING_MESSAGES_QUERY_KEY} from "../../../../queries/useGetPromotingMessages.ts";
...
export const PromotingTab = () => {
    ...
    const eventTitleById = useMemo(() => {
        ...
        (allEventsData?.data ?? []).forEach(e => { map[String(e.id)] = e.title; });
        return map;
    }, [allEventsData]);
    ...
    const messagesQuery = useGetPromotingMessages(eventId, searchParams);
    ...
    const handleToggleResolved = (msg: OutgoingMessage) => {
        ...
        resolveMutation.mutate({
            ...
            onSuccess: () => {
                queryClient.invalidateQueries({queryKey: [GET_PROMOTING_MESSAGES_QUERY_KEY]});
                setResolvingId(null);
            },
            onError: () => {
    ...
    const getActionButton = (msg: OutgoingMessage) => {
        ...
        return (
            <Button
                size="compact-xs"
                variant="light"
                onClick={() => setResolveModalMessage({
                    issue: toDeliveryIssue(msg),
                    audienceEventId,
                })}
            >
                {label}
        ...
        <>
            ...
            {resolveModalMessage && (
                <ResolveDeliveryIssueModal
                    ...
                    eventId={resolveModalMessage.audienceEventId}
